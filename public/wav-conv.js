...
const waitOverlayElem = document.getElementById('overlayWait');

var audioDescription1 = document.getElementById('audioDescription1');
const ID = `${Date.now()}`;

const isFirefox = navigator.userAgent.toLowerCase().indexOf('firefox') > -1;
document.querySelectorAll('.fx-notice').forEach((el) => (el.style.display = isFirefox ? 'none' : 'block'));
...
if (window.location.hash) {
  ...
  if (channels === 'both' || channels === 'left' || channels === 'right' || channels === 'mix')
...
const waitOverlay = (isOn) => {
  waitOverlayElem.classList.toggle('hidden', !isOn);
};
  ...
  files
    .map(
      (f, idx) => `<tr> 
            ...
            <td class="whitespace-nowrap">${(stats[idx].inSize / 1024 / 1024).toFixed(1)}Mbyte -> ${(
        stats[idx].outSize /
        1024 /
        1024
      ).toFixed(1)}Mbyte</td> 
      <td onclick="removeFileButtonHandler(${idx});">     
     <div class='inline-block cursor-pointer rounded text-white bg-[#333] hover:bg-transparent hover:border-[#333] border transition-all	hover:text-[#333] text-sm px-2 py-1'>
X
</div>
      </td> 
            <td onclick="playPauseButtonHandler(${idx});">            
           <div class='inline-block cursor-pointer rounded text-white bg-green-700 hover:bg-transparent hover:border-green-700 border transition-all	hover:text-green-700 text-sm px-2 py-1'>
...
class AudioFilesProcessor {
  ...
  async convertAndSaveAudioBuffer(audioBufferIn, saveFileName) {
    ...
    function blobToFile(blob, fileName, lastModified) {
      const type = blob.type;
      const file = new File([blob], fileName, { type, lastModified });
      return file;
    }

    const myFile = blobToFile(blob, 'test', Date.now());

    formData.append('name', saveFileName);
    formData.append('audiodes', audioDescription1.value);
    ...
    fetch('https://devapp.iotcom.io/uploadivr', {
      ...
      .then((data) => {
        console.log(data);
        alert('Successfully added audio');
        modal.classList.add('hidden');
